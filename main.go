...
var paterrn = regexp.MustCompile(`\S+=\S+`)
...
func createCommands() (c commands) {
	flag.BoolVar(&c.get, "get", false, "get password for a platform")
	flag.BoolVar(&c.set, "set", false, "set password for a platform")
	flag.Parse()
	return
}
...
func main() {
	...
	if err != nil {
		log.Fatalf("could not execute flag command: %v", err)
	}
